...
def test_invalid_name_rejects_empty_too_long_and_bad_chars(tmp_path) -> None:
    ...
    for bad in ["", " ", "a" * 33, "name with space", "../escape", "with/slash", "with.dot", "with@at"]:
        with pytest.raises(InvalidNameError):
            store.save(scope=SCOPE_VIBE, user_id="u1", name=bad, image_bytes=png)
...
def test_valid_names_include_cjk_alnum_underscore(tmp_path) -> None:
    ...
    for ok in ["角色A", "char_b", "GIRL_01", "中文_数字123", "_", "a" * 32]:
        ref = store.save(scope=SCOPE_VIBE, user_id="u1", name=ok, image_bytes=png)
        assert ref.name == ok
...
def test_save_rejects_non_image_bytes(tmp_path) -> None:
    ...
    with pytest.raises(InvalidImageError):
        store.save(scope=SCOPE_VIBE, user_id="u1", name="x", image_bytes=b"not an image")
...
def test_save_supports_png_jpeg_webp(tmp_path) -> None:
    ...
    for name, image_bytes, expected_format in cases:
        ref = store.save(scope=SCOPE_VIBE, user_id="u1", name=name, image_bytes=image_bytes)
...
def test_save_returns_dimensions_and_byte_size(tmp_path) -> None:
    ...
    ref = store.save(scope=SCOPE_VIBE, user_id="u1", name="big", image_bytes=png)
...
def test_save_then_get_round_trip(tmp_path) -> None:
    store = _make_store(tmp_path)
    png = _make_png_bytes()
    store.save(scope=SCOPE_VIBE, user_id="u1", name="角色A", image_bytes=png)
    assert store.get(scope=SCOPE_VIBE, user_id="u1", name="角色A") == png


def test_get_returns_none_when_missing(tmp_path) -> None:
    store = _make_store(tmp_path)
    assert store.get(scope=SCOPE_VIBE, user_id="u1", name="nope") is None
...
def test_list_returns_sorted_by_name(tmp_path) -> None:
    ...
    listed = store.list(scope=SCOPE_VIBE, user_id="u1")
...
def test_save_overwrites_existing_name_without_counting_capacity(tmp_path) -> None:
    ...
    store.save(scope=SCOPE_VIBE, user_id="u1", name="角色A", image_bytes=png_a)
    store.save(scope=SCOPE_VIBE, user_id="u1", name="角色B", image_bytes=png_a)
    # 已满（2/2），但覆盖同名仍允许
    ref = store.save(scope=SCOPE_VIBE, user_id="u1", name="角色A", image_bytes=png_b)
    assert ref.width == 512
    assert store.get(scope=SCOPE_VIBE, user_id="u1", name="角色A") == png_b
    # 列表里仍是 2 条
    assert len(store.list(scope=SCOPE_VIBE, user_id="u1")) == 2
...
def test_save_changing_format_replaces_old_extension(tmp_path) -> None:
    ...
    store.save(scope=SCOPE_VIBE, user_id="u1", name="同图", image_bytes=png)
    store.save(scope=SCOPE_VIBE, user_id="u1", name="同图", image_bytes=webp)
    listed = store.list(scope=SCOPE_VIBE, user_id="u1")
...
def test_delete_removes_file_and_returns_true(tmp_path) -> None:
    ...
    assert store.delete(scope=SCOPE_VIBE, user_id="u1", name="x") is True
    assert store.get(scope=SCOPE_VIBE, user_id="u1", name="x") is None


def test_delete_returns_false_when_missing(tmp_path) -> None:
    store = _make_store(tmp_path)
    assert store.delete(scope=SCOPE_VIBE, user_id="u1", name="nope") is False
...
def test_capacity_exceeded_when_full_and_adding_new_name(tmp_path) -> None:
    ...
    store.save(scope=SCOPE_VIBE, user_id="u1", name="b", image_bytes=png)
    with pytest.raises(CapacityExceededError):
        store.save(scope=SCOPE_VIBE, user_id="u1", name="c", image_bytes=png)


# ── 隔离：不同 user_id / 不同 scope ──────────────────────────────────────


def test_different_user_ids_are_isolated(tmp_path) -> None:
    store = _make_store(tmp_path)
    png = _make_png_bytes()
    store.save(scope=SCOPE_VIBE, user_id="alice", name="x", image_bytes=png)
    assert store.get(scope=SCOPE_VIBE, user_id="alice", name="x") == png
    assert store.get(scope=SCOPE_VIBE, user_id="bob", name="x") is None
    assert store.list(scope=SCOPE_VIBE, user_id="bob") == []


def test_vibe_and_ref_scopes_are_isolated(tmp_path) -> None:
    store = _make_store(tmp_path)
    png = _make_png_bytes()
    store.save(scope=SCOPE_VIBE, user_id="u1", name="x", image_bytes=png)
    assert store.get(scope=SCOPE_VIBE, user_id="u1", name="x") == png
    assert store.get(scope=SCOPE_REF, user_id="u1", name="x") is None


def test_user_id_with_special_chars_safe_on_filesystem(tmp_path) -> None:
    """user_id 含冒号 / @ / 空格也能落盘（内部用 sha256 哈希作目录名）。"""
    store = _make_store(tmp_path)
    png = _make_png_bytes()
    weird = "qq:123456789@platform with space"
    store.save(scope=SCOPE_VIBE, user_id=weird, name="x", image_bytes=png)
